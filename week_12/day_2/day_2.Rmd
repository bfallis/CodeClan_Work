...
t.test(x = herb$sleep_total, y = omni$sleep_total, alternative = "greater", paired = FALSE)
...
# Success criteria
0.0005  * nrow(msleep) >= 10
(1 - 0.0005) * nrow(msleep) >= 10

binom.test(x = nrow(num_domesticated), n = nrow(msleep), p = 0.0005, alternative = "greater")
# p value < 0.05 so can reject Ho
```
...
wp.t(n1 = NULL, d = effect_size, alpha = 0.05, power = 0.95,
        type = "two.sample", alternative = "two.sided")
# need 77 in each group if equal size
```
